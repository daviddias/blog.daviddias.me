...
One of the tools we can use for the recon segment during a penetration test is [`DirBuster`](https://www.owasp.org/index.php/Category:OWASP_DirBuster_Project), now part of the [`OWASP ZAP Attack Proxy`](https://www.owasp.org/index.php/ZAP). DirBuster is:

> DirBuster is a multi threaded java application designed to brute force directories and files names on web/application servers.

In essence, what DirBuster does is grabbing a list of words, throwing them at a Web Server as possible paths for a Web Application and sees the Web Server response to conclude if there is a resource available in that path. This can be most useful for all of the URLs that are not linked in any of the pages/resource of the Web Application and that therefore a crawler would never get to them, this pages/resource can sometimes be an hidden admin panel or a old/new version of the application that is poorly authenticated or simply exposes interesting information to an attacker. So, a visual representation would be:

```
┌─────────┐   ┌─────────┐   ┌─────────┐
│ List of │   │ Attempt │   │ Output  │
│  words  ├──▶│ request │──▶│ matches │
└─────────┘   └─────────┘   └─────────┘
```

This looks like a perfect scenario to use `streams`! And that is what we are going to explore, using Node.js :). 

# Streams

Streams are one of Node.js' primer, they are part of the core API, offering an amazing way to control flow and avoid backpressure.

If you aren't familiar with Node.js streams API, I totally recommend you to check it out, to get inspired, consider watching the following two talks, they make an amazing job explaining the amazing powers of streams
...
# DirBuster architecture

For the Node.js DirBuster implementation, we want it to be able to:
- Parse a file with a list of words
- Identify which paths are dirs
- Test every path to find which of them represent a resource in the Web Application
...
To build the dir testing component, we need to build a "stream feedback loop", this might not look natural at first, but let me explain:
...
Confusing? Let's see in a graph:
```
               ┌──────────┐  ┌─────────┐
...
![](https://cldup.com/mkxWTwUcD_-1200x1200.png)
